"""
Eval bottleneck_expert_positive_negative — termination_study stage 1 extension.

Evaluates checkpoints at epochs [50, 100, 150, 200, 250]. Incremental: skips already-evaluated
(epoch) pairs. Safe to re-run.

Usage (repo root):
  modal run --detach \\
    experiments/final_experiments/pusht/termination_study/stage1_bottleneck_source/modal_eval_stage1_pn.py

Results saved to:
  stage1_pn_results.json  (local)
  Modal Volume: .../termination_study/stage1/stage1_pn_results.json
"""
...
RUN_LABEL = "bottleneck_expert_positive_negative"
LOW_DIR = f"/experiments/final_experiments/pusht/termination_study/stage1/{RUN_LABEL}"
CONFIG = "configs/pusht/exp_term_bottleneck_expert_positive_negative.yaml"

OUTPUT_PATH = Path(
    "experiments/final_experiments/pusht/termination_study/stage1_bottleneck_source/stage1_pn_results.json"
)
VOLUME_OUTPUT_PATH = (
    f"{EXPERIMENTS_MOUNT}/final_experiments/pusht/termination_study/stage1/stage1_pn_results.json"
)
...
def _eval_aggregate(
    ...
    """Spawn rollouts, collect results, save JSON to Modal Volume."""
    import json
    import numpy as np
    from pathlib import Path
...
def main(n_action_steps: int = 8) -> None:
    ...
    print(f"\nSubmitting {len(work_items)} eval jobs...")
