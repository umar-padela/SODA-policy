"""
Eval bottleneck_expert and bottleneck_ddim_positive — termination_study stage 1 eval.
...
can be closed after spawning. Use --detach to return immediately:
  ...
  modal run --detach experiments/final_experiments/pusht/termination_study/stage1_bottleneck_source/modal_eval_stage1.py
...
def _run_plot(output_path: Path) -> None:
    plot_script = Path(__file__).parent / "plot_stage1.py"
...
def _eval_aggregate_stage1(
    ...
    stage1_results = {k: list(v) for k, v in existing.items()}
    ...
    for item, call in calls:
        ...
        epoch = item["epoch"]
        stage1_results.setdefault(run_label, []).append({
            "epoch": epoch,
        ...
        for k in stage1_results:
            stage1_results[k].sort(key=lambda r: r["epoch"])
        best_per_run = {k: max(v, key=lambda r: r["mean_score"]) for k, v in stage1_results.items() if v}
    ...
    best_per_run = {k: max(v, key=lambda r: r["mean_score"]) for k, v in stage1_results.items() if v}
    winner = max(best_per_run, key=lambda k: best_per_run[k]["mean_score"]) if best_per_run else None
    output = {"n_action_steps": n_action_steps, "results": stage1_results, "best_per_run": best_per_run, "winner": winner}
...
def main(n_action_steps: int = 8) -> None:
    ...
    work_items = []
    for run_label, run_info in STAGE1_RUNS.items():
    ...
    agg_call = _eval_aggregate_stage1.spawn(work_items, stage1_results, n_action_steps)
    result = agg_call.get()
